...
<div class="posts">
    ...
    <?php foreach ($posts as $post) {
        ...
        if (validate_array($post)) {
            ...
            // We check if the post is a draft and if it's owned by the current user OR if the current logged in user is the admin account.          
            if (


                (isset($post['draft']) && !$post['draft'])

                ||

                (isset($_SESSION['CURRENT_USER_ID']) && $_SESSION['CURRENT_USER_ID'] == $post['owner']['id'])
                ...
                // We print the title
                if ($post['draft']) {
                    echo "<h3><a href='" . $post_link . "'>" . $post_title . "</a> <span class='text-danger'>(Draft)</span></h3>";
                } else {
